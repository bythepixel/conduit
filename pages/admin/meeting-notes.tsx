...
export default function MeetingNotes() {
    ...
    const handleLinkCompany = async (noteId: number, companyId: number | null) => {
        setLinkingNote(noteId)
        try {
            const res = await fetch(`/api/meeting-notes/${noteId}/link`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ hubspotCompanyId: companyId })
            })
            const data = await res.json()
            ...
            if (res.ok) {
                ...
                setNotes(prevNotes => prevNotes.map(n =>
                    n.id === noteId ? { ...n, hubspotCompany: data.note.hubspotCompany, hubspotCompanyId: data.note.hubspotCompanyId } : n
                ))
            } else {
                setErrorModal({
                    isOpen: true,
                    message: data.error || 'Failed to link company',
                    type: 'error'
                })
            }
        } catch (error: any) {
            setErrorModal({
                isOpen: true,
                message: 'An error occurred while linking: ' + error.message,
                type: 'error'
            })
        } finally {
            setLinkingNote(null)
                                ...
                                <div className="mb-3">
                                    ...
                                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                                        {note.hubspotCompany ? (
                                            ...
                                            const suggested = findSuggestedCompany(note.title)
                                            if (suggested) {
                                                const isLinking = linkingNote === note.id
                                                return (
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <button
                                                            onClick={(e) => {
                                                                e.stopPropagation()
                                                                handleLinkCompany(note.id, suggested.id)
                                                            }}
                                                            disabled={isLinking}
                                                            className="px-1.5 py-0.5 bg-amber-600/20 hover:bg-amber-600 text-amber-400 hover:text-white border border-amber-600/30 rounded text-[10px] font-bold transition-all flex items-center gap-1 active:scale-95"
                                                        >
                                                            {isLinking ? (
